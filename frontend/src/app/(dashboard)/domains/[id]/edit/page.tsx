...
export default function EditDomainPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await api.put<Domain>(`/domains/${params.id}`, { name, description: description || undefined })
    if (res.success) {
      router.push(`/domains/${params.id}`)
    } else {
  ...
  return (
    <div className="max-w-lg">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">Editar Dominio</h1>
