...
export default function NewSchedulePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (scheduleType === 'RECURRING') {
      if (!cronExpression) {
        setError('Expressao cron e obrigatoria para agendamentos recorrentes')
        setLoading(false)
        return
      }
      ...
      if (!nextRunAt) {
        setError('Data/hora e obrigatoria para agendamento unico')
        setLoading(false)
        return
      }
    ...
    if (res.success) {
      ...
      setError(res.error?.message || 'Falha ao criar agendamento')
    }
    setLoading(false)
  }
  ...
  return (
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white p-6 rounded-xl shadow-sm border border-gray-200">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Teste</label>
        ...
        <div>
          ...
          <div className="flex space-x-4">
            <label className="flex items-center">
              ...
              <span className="text-sm text-gray-700">Unico</span>
            </label>
            <label className="flex items-center">
        ...
        {scheduleType === 'ONCE' ? (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-gray-400">Formato: minuto hora dia mes dia_semana</p>
          </div>
        )}
